...
export default function MemberDetailPage({
  ...
  const activeSubscription =
    member.subscriptions?.find((sub) => sub.status === 'ACTIVE' && new Date(sub.startDate) <= now) ||
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start max-w-7xl mx-auto p-4">
        ...
        <MemberProfileCard 
          member={member} 
          displayStatus={STATUS_TRANSLATIONS[memberState] || memberState} 
          safeStatusStyles={STATUS_STYLES[memberState] || STATUS_STYLES['INACTIVE']} 
